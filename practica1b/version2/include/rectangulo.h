...
public:
    Rectangulo();
    Rectangulo(Punto piz, Punto pdr);
    Punto getX() const;
    Punto getY() const;
    ...
    double area() const;                      // Recibe un dato de tipo Rectangulo y devuelve su area
    double perimetro() const;                 // recibe un dato de tipo Rectangulo y devuelve su per´ımetro.
    bool esMayor(const Rectangulo &r2) const; // recibe dos datos r1,r2 de tipo Rectangulo y devuelve verdadero si el area de r1 es mayor que el de r2. Devuelve falso en caso contrario.
    bool estaDentro(const Punto &p) const;    // estaDentro: recibe un dato r de tipo Rectangulo y un dato p detipo Punto2D. Devuelve verdadero si p ”esta dentro” del rect ´ angulo ´r y falso en caso contrario.
};
#endif /* RECTANGULO_H */