...
void redimensionar_indices(Diccionario &dic){
    ...
    for (int i = 0; i <= N_INDICE; i++) {
        ...
        if (i < N_INDICE) {
            char letra = 'A' + i;
            while (k < dic.util && dic.datos[k][0] == letra) {//dic.datos[k][0] es la primera letra de la palabra contenida en datos[k]
                k++;
            }
        }
    }
...
void redimensionar(Diccionario &dic, int tipo){
    ...
    if(tipo==1){
        ...
        //entonces hara falta añadir mas espacio
         if (dic.util == dic.cap) {//estas condiciones tienen que estar fuera de las funciones
...
void agregaPalabra(Diccionario &dic, string pal){
    ...
    if (existe(dic, pal)){
        ...
        pos =  posF - dic.datos; // posicion de la palabra que se quiere agregar
