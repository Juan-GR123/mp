...
void ordenar() {
    ...
    //1. Crear un array dinamico v de 10 polígonos.
    ...
    for(int j=0; j<9; j++){// esto es por si el perimetro de uno del final es menor que uno del principio del array es decir, para que compare varias veces
        for(int i=0; i<9; i++){
            if(v[i] < v[i+1]){ //operator < polireg ya compara perimetros
                PoliReg cambio= v[i];
                v[i] = v[i+1];
                v[i+1] = cambio;
            }
