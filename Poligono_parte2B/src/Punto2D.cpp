...
void Punto2D::setX(float _x){
     x=_x;
}

void Punto2D::setY(float _y){
    y=_y;
}
...
Punto2D& Punto2D::operator =(const Punto2D &rhs){
    this->setXY(rhs.x, rhs.y);
    
    return (*this);
            
}

istream& operator>>(istream &flujo, Punto2D &p){

    
    char espacio;
    float Nx, Ny;
    
    flujo >> espacio;// los parentesis
    flujo >> Nx;
    flujo >> espacio; // la ,
    flujo >> Ny;
    flujo >> espacio;
